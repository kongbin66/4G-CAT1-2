...
void ds1302_task(void *parameter)
{
  ...
  while (1)
  {
    ...
    //Serial.printf("now_unixtime=%d",now_unixtime);
      ...
      if ((f_Flight_Mode == false) && (f_lose == false) && (workingState = WORKING)) //正常记录和发送
      {
        ...
        if (client.connected())
        {
          ...
          if (sendTempAndHumi3()) //数据发送成功
          {
            ...
            Serial.printf("sleep tnow_unixtime - last_rec_stamp=(%d-%d),time jiange:%d\n", now_unixtime,last_rec_stamp,(now_unixtime - last_rec_stamp));
          ...
          else //数据发送失败
          {
            ...
            eeprom_config_save_parameter();                               // screen_loopEnabled = true;
            screen_loopEnabled = true;
            screen_On_Start = sys_sec;
            screen_On_now = sys_sec;
            digitalWrite(MODEM_POWER_ON, LOW);
